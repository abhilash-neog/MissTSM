...
#             os.makedirs(folder_path)
        ...
        # -----------------------------
        print(f"Masks_y shape: {masks_y.shape}")
        print(f"Non-zero mask ratio: {masks_y.sum() / masks_y.size}")
        print(f"Any completely masked sequences: {(masks_y.sum(axis=(1,2)) == 0).any()}")
        
        # Debug the NaN issue
        print(f"Preds shape: {preds.shape}")
        print(f"Preds contains NaN: {np.isnan(preds).any()}")
        print(f"Preds contains Inf: {np.isinf(preds).any()}")
        print(f"Preds min/max: {np.nanmin(preds):.4f}/{np.nanmax(preds):.4f}")
        
        print(f"Trues contains NaN: {np.isnan(trues).any()}")
        print(f"Trues min/max: {np.nanmin(trues):.4f}/{np.nanmax(trues):.4f}")
        
        # Check which batches have NaN predictions
        nan_mask = np.isnan(preds)
        nan_batches = np.any(nan_mask, axis=(1,2))
        print(f"Number of batches with NaN: {nan_batches.sum()}")
        # -----------------------------
